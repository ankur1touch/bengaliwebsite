...
export default function HeroSlider({ initialMatches, initialNews }: HeroSliderProps) {
  ...
  const { matches: reduxMatches, status: matchStatus } = useAppSelector((s) => s.matches);
  const { articles, status: newsStatus } = useAppSelector((s) => s.news);
  const t = useTranslations('home');
  const [active, setActive] = useState(0);
  const countdown = useCountdown(WC_KICKOFF);

  const matches = reduxMatches.length ? reduxMatches : (initialMatches ?? []);
  const newsList = articles.length ? articles : (initialNews ?? []);

  useEffect(() => {
    if (matchStatus === 'idle' && !initialMatches?.length) dispatch(fetchMatches());
    if (newsStatus === 'idle' && !initialNews?.length) dispatch(fetchNews());
  }, [dispatch, matchStatus, newsStatus, initialMatches?.length, initialNews?.length]);
  ...
  return (
    <section className="relative w-full overflow-hidden bg-brand-navy">
      <div className="relative min-h-[420px] sm:min-h-[480px] lg:min-h-[520px]">
        {/* Slide 1 — World Cup 2026 (LCP target) */}
        <div
        ...
        >
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 h-full flex flex-col justify-center py-16 sm:py-20">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3 mb-8">
              <CountdownBox value={countdown.days} label={t('heroDays')} />
              <span className="hidden sm:flex items-center text-yellow-400/60 font-display text-2xl pb-6">:</span>
              <CountdownBox value={countdown.hours} label={t('heroHours')} />
              <span className="hidden sm:flex items-center text-yellow-400/60 font-display text-2xl pb-6">:</span>
              <CountdownBox value={countdown.mins} label={t('heroMins')} />
              <span className="hidden sm:flex items-center text-yellow-400/60 font-display text-2xl pb-6">:</span>
              <CountdownBox value={countdown.secs} label={t('heroSecs')} />
            </div>
            <Link href="/world-cup">
        ...
        <div
          className={`absolute inset-0 transition-opacity duration-700 ${
            active === 2 ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'
          }`}
        >
          {topStory?.imageUrl ? (
            <Image
