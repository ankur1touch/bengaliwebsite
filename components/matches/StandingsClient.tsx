...
const LEAGUES = [
  { id: LEAGUE_IDS.LA_LIGA,          label: 'La Liga' },
  { id: LEAGUE_IDS.PREMIER_LEAGUE,  label: 'Premier League' },
  { id: LEAGUE_IDS.CHAMPIONS_LEAGUE, label: 'Champions League' },
  { id: LEAGUE_IDS.SERIE_A,         label: 'Serie A' },
  { id: LEAGUE_IDS.BUNDESLIGA,      label: 'Bundesliga' },
];
...
export default function StandingsClient() {
  ...
  const [leagueId, setLeagueId] = useState<number>(LEAGUE_IDS.LA_LIGA);

  useEffect(() => {
    dispatch(fetchRankings({ leagueId }));
  }, [dispatch, leagueId]);

  if (status === 'loading' || status === 'idle') {
    return <div className="space-y-2">{[...Array(20)].map((_, i) => <Skeleton key={i} className="h-10" />)}</div>;
  }
  if (status === 'failed') return <ErrorState message={error ?? undefined} onRetry={() => dispatch(fetchRankings({ leagueId }))} />;
